...
def main():
    ...
    # Hib-specific deaths aren't recorded (coded under meningitis), so the Deaths view
    # uses the MEASURED childhood-meningitis death rate (ages 1-4, NCHS HIST001R) as the
    # proxy — the same real series shown in the Hib deep-dive card — instead of a
    # back-calculated cases x CFR estimate. Cases/incidence stay as invasive-disease est.
    men = {int(r["year"]): float(r["age1_4"]) for r in read_csv("childhood_meningitis_death_rates.csv")}
    by_year = {r["year"]: r for r in data["hib"]}
    for rec in by_year.values():        # clear all back-calculated death estimates first
        rec["death_rate"] = None
        rec["deaths"] = None
        rec["cfr"] = None
    for y, rate in men.items():
        ...
        rec["cfr"] = None
    data["hib"] = [by_year[y] for y in sorted(by_year)]
